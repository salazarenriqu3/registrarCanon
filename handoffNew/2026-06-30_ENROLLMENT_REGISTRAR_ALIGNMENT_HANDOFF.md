...
| Payment keying | Enrollment may re-key `payments.reference_number` from applicant ref to student number after issuance. Registrar payment/status reads must accept either key so the applicant detail screen does not under-report paid totals after handoff. |
...
- Cashier/finalization may write enrollment state when the payment gate is satisfied.
- Ledger inspection must not mutate student status as a side effect of lookup.
