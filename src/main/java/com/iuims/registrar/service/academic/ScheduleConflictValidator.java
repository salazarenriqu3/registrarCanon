...
public final class ScheduleConflictValidator {
    ...
    public String validateNewSlot(int sectionId, Integer facultyId, Integer roomId,
        ...
        List<Map<String, Object>> sections = db.queryForList(
            "SELECT term_id, section_code, block_id FROM class_sections WHERE section_id = ?", sectionId);
        if (sections.isEmpty()) {
            return "Section not found.";
        }

        int termId = ((Number) sections.get(0).get("term_id")).intValue();
        String targetSectionCode = sectionCode(sections.get(0));
        Integer targetBlockId = sections.get(0).get("block_id") instanceof Number n ? n.intValue() : null;
        if (roomId != null) {
        ...
        String overlapSql =
            "SELECT cs.section_id, cs.section_code, c.course_code FROM class_schedules sch " +
        ...
        Map<String, Object> blockConflict = findSameBlockConflict(
            termId, sectionId, targetBlockId, targetSectionCode, dayOfWeek, startTime, endTime);
        if (blockConflict != null) {
            return "Block cohort " + targetSectionCode + " already has another subject at that time ("
                + courseLabel(blockConflict) + ").";
        }
    ...
    public ConflictPreview findExistingConflictPreview(int termId, int maxResults) {
        ...
        int blockLimit = safeLimit >= 3 ? Math.max(1, safeLimit / 3) : 0;
        int roomLimit = Math.max(1, (safeLimit - blockLimit + 1) / 2);
        int facultyLimit = Math.max(1, safeLimit - blockLimit - roomLimit);

        List<Map<String, Object>> blockConflicts = blockLimit > 0
            ? db.queryForList(
                "SELECT 'BLOCK_COHORT' AS conflict_type, a.section_code AS section_a, b.section_code AS section_b, " +
                "COALESCE(a.section_code, CONCAT('Block #', COALESCE(a.block_id, 0))) AS resource_name, " +
                "ca.course_code AS course_a, cb.course_code AS course_b, s1.day_of_week, s1.start_time, s1.end_time " +
                ...
                "JOIN courses ca ON ca.course_id = a.course_id " +
                "JOIN courses cb ON cb.course_id = b.course_id " +
                "WHERE a.term_id = ? AND a.section_id <> b.section_id AND (" +
                "  (a.block_id IS NOT NULL AND a.block_id = b.block_id) OR " +
                "  (a.block_id IS NULL AND b.block_id IS NULL AND a.section_code = b.section_code AND a.section_code REGEXP ?)" +
                ") LIMIT ?",
                termId, BlockOfferingService.BLOCK_SECTION_CODE_PATTERN, blockLimit + 1)
            : List.of();
        ...
        boolean truncated = blockConflicts.size() > blockLimit
            || roomConflicts.size() > roomLimit
            || facultyConflicts.size() > facultyLimit;
        List<Map<String, Object>> preview = new ArrayList<>(safeLimit);
        preview.addAll(blockConflicts.subList(0, Math.min(blockLimit, blockConflicts.size())));
    ...
    private String courseLabel(Map<String, Object> row) {
        Object value = row.get("course_code");
        return value == null ? sectionCode(row) : String.valueOf(value);
    }

    private Map<String, Object> findSameBlockConflict(int termId,
                                                      int sectionId,
                                                      Integer blockId,
                                                      String sectionCode,
                                                      int dayOfWeek,
                                                      ...
                                                      LocalTime endTime) {
        if (blockId != null) {
            List<Map<String, Object>> blockConflicts = db.queryForList(
                "SELECT cs.section_id, cs.section_code, c.course_code FROM class_schedules sch " +
                    "JOIN class_sections cs ON cs.section_id = sch.section_id " +
                    "JOIN courses c ON c.course_id = cs.course_id " +
                    "WHERE cs.term_id = ? AND cs.block_id = ? AND cs.section_id <> ? " +
                    "AND sch.day_of_week = ? AND sch.start_time < ? AND sch.end_time > ? LIMIT 1",
                termId, blockId, sectionId, dayOfWeek, endTime, startTime);
            if (!blockConflicts.isEmpty()) {
                return blockConflicts.get(0);
            }
        }
        ...
        List<Map<String, Object>> sectionCodeConflicts = db.queryForList(
            "SELECT cs.section_id, cs.section_code, c.course_code FROM class_schedules sch " +
                "JOIN class_sections cs ON cs.section_id = sch.section_id " +
                "JOIN courses c ON c.course_id = cs.course_id " +
                "WHERE cs.term_id = ? AND cs.section_code = ? AND cs.section_id <> ? " +
                "AND sch.day_of_week = ? AND sch.start_time < ? AND sch.end_time > ? LIMIT 1",
            termId, sectionCode, sectionId, dayOfWeek, endTime, startTime);
        return sectionCodeConflicts.isEmpty() ? null : sectionCodeConflicts.get(0);
    }
