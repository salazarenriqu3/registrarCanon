...
import com.iuims.registrar.entity.ProgramFeeSetting;
import com.iuims.registrar.repository.ProgramFeeSettingRepository;
...
public class TermFeeAdminService {
    ...
    public Map<String, Object> buildTermReadinessSummary(Integer termId) {
        ...
        summary.put("readyFeeScopeCount", 0);
        ...
        for (Map<String, Object> program : programs) {
            ...
            for (int yearLevel = 1; yearLevel <= 4; yearLevel++) {
                for (int semester = 1; semester <= 2; semester++) {
                    ...
                    String status = resolveScopeStatus(exactOpt);
        ...
        summary.put("readyFeeScopeCount", readyFeeScopeCount);
        ...
        for (Map<String, Object> program : listPrograms()) {
            ...
            for (int yearLevel = 1; yearLevel <= 4; yearLevel++) {
                for (int semester = 1; semester <= 2; semester++) {
                    ...
                    row.put("sourceHint", buildSourceHint(status, templateSourceCode));
                    row.put("actionHint", buildActionHint(status));
    ...
    private String resolveScopeStatus(Optional<ProgramFeeSetting> exactOpt) {
    ...
    private String buildSourceHint(String status, String templateSourceCode) {
        ...
        if (templateSourceCode != null && !templateSourceCode.isBlank()) {
            return "No exact active-term row. Official data must come from Enrollment3 for " + templateSourceCode + ".";
        }
        return "No exact active-term row. Official data must be created in Enrollment3.";
    }
    ...
    private String buildActionHint(String status) {
        ...
        if ("INCOMPLETE".equals(status)) {
            return "Enrollment3 must complete the canonical fee row before this scope can be trusted.";
        }
        return "No Registrar action. Enrollment3 must supply the exact canonical fee row.";
    }
