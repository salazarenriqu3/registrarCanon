...
@Service
public class WithdrawalService implements TermRolloverDispositionService.WithdrawalDispositionBridge {
    ...
    private final ObjectProvider<ScholarEnrollmentService> scholarEnrollmentServiceProvider;
    ...
    private boolean referenceDataBootstrapEnabled = true;

    @Autowired
    public WithdrawalService(JdbcTemplate db, ObjectProvider<ScholarEnrollmentService> scholarEnrollmentServiceProvider,
        ...
        this.db = db;
        this.scholarEnrollmentService = null;
        this.scholarEnrollmentServiceProvider = scholarEnrollmentServiceProvider;
    ...
    public WithdrawalService(JdbcTemplate db, ScholarEnrollmentService scholarEnrollmentService,
        ...
        this.scholarEnrollmentServiceProvider = null;
        this.documentTrailService = documentTrailService;
        this.regFormEventService = regFormEventService;
        this.globalTermService = globalTermService;
        this.enlistmentSchemaService = enlistmentSchemaService;
    ...
    private ScholarEnrollmentService scholarEnrollmentService() {
        return scholarEnrollmentService != null ? scholarEnrollmentService : (scholarEnrollmentServiceProvider != null ? scholarEnrollmentServiceProvider.getIfAvailable() : null);
    }
    ...
    public List<Map<String, Object>> listStudentRequests(String studentNumber) {
        ...
        return db.queryForList(baseRequestSql() +
                "WHERE wr.student_number COLLATE utf8mb4_unicode_ci = ? ORDER BY wr.requested_at DESC, wr.request_id DESC",
            studentNumber.trim());
    }
    ...
    public List<Map<String, Object>> listRequests(String status) {
        ...
        return db.queryForList(baseRequestSql() +
                "WHERE wr.status COLLATE utf8mb4_unicode_ci = ? ORDER BY wr.requested_at ASC, wr.request_id ASC",
    ...
    private String baseRequestSql() {
        return """
            SELECT wr.request_id, wr.student_number, wr.archive_key,
                   COALESCE(s.real_name, wr.student_number COLLATE utf8mb4_unicode_ci) AS student_name,
            ...
            LEFT JOIN students s
                   ON s.student_number = wr.student_number COLLATE utf8mb4_unicode_ci
    ...
    public DirectDropResult dropSubjectByRegistrar(String studentNumber, Integer sectionId,
        ...
        double flatFee = resolveFlatSubjectDropFee(sn);
        List<Map<String, Object>> bundleEnlistments = resolveSubjectDropBundle(sn, currentTermId, enlistment);
        List<Integer> sectionIds = new ArrayList<>();
        List<WithdrawalPolicySnapshot> policies = new ArrayList<>();
        for (Map<String, Object> bundleEnlistment : bundleEnlistments) {
            sectionIds.add(((Number) bundleEnlistment.get("section_id")).intValue());
            policies.add(requireAllowedPolicy(sn, bundleEnlistment));
        }

        supersedePendingRequests(sn, sectionIds, actedBy);
        List<WithdrawalPolicySnapshot> flatPolicies = new ArrayList<>();
        for (int i = 0; i < policies.size(); i++) {
            WithdrawalPolicySnapshot base = policies.get(i);
            flatPolicies.add(flatPolicySnapshot(
                base,
                i == 0 ? flatFee : 0.0,
                i == 0 ? "Registrar flat subject-drop fee" : "Bundled corequisite subject-drop archive line"));
        }
        long requestId = insertCompletedDirectDrop(
            sn, REASON_CLASS_DROP, remarks, actedBy, "SINGLE_SUBJECT", bundleEnlistments, flatPolicies);

        for (int i = 0; i < bundleEnlistments.size(); i++) {
            Map<String, Object> bundleEnlistment = bundleEnlistments.get(i);
            WithdrawalPolicySnapshot flatPolicy = flatPolicies.get(i);
            scholarEnrollmentService().dropSubjectByEnlistmentId(
                ((Number) bundleEnlistment.get("enlistment_id")).longValue(),
                flatPolicy.estimatedCharge(),
                flatPolicy.policyNote());
        }
        recordDirectDropEvents(sn, requestId, actedBy, "SINGLE_SUBJECT", bundleEnlistments,
            flatFee, remarks);
        return new DirectDropResult(requestId, bundleEnlistments.size(), flatFee, "SINGLE_SUBJECT");
    }
    ...
    public DirectDropResult dropStudentByRegistrar(String studentNumber, String reasonCode,
        ...
        int residualPurged = purgeResidualCurrentTermEnlistments(sn, currentTermId, actedBy, "FULL_CURRENT_TERM");
        persistOutstandingBalanceHold(requestId, sn);
        markStudentWithdrawn(sn);
        recordDirectDropEvents(sn, requestId, actedBy, "FULL_CURRENT_TERM", currentLoad, totalCharge, remarks);
        return new DirectDropResult(requestId, currentLoad.size() + residualPurged, totalCharge, "FULL_CURRENT_TERM");
    }

    @Transactional
    public DirectDropResult clearCurrentTermLoadForProgramShift(String studentNumber, String remarks, String actor) {
        return clearCurrentTermLoadForProgramShift(studentNumber, "SHIFTING", remarks, actor, true);
    }

    @Transactional
    public DirectDropResult clearCurrentTermLoadForProgramShift(String studentNumber, String reasonCode,
                                                                String remarks, String actor) {
        return clearCurrentTermLoadForProgramShift(studentNumber, reasonCode, remarks, actor, true);
    }

    @Transactional
    public DirectDropResult clearCurrentTermLoadForCompositeProgramShift(String studentNumber,
                                                                          String remarks,
                                                                          String actor) {
        return clearCurrentTermLoadForProgramShift(studentNumber, "SHIFTING", remarks, actor, false);
    }

    private DirectDropResult clearCurrentTermLoadForProgramShift(String studentNumber,
                                                                  String reasonCode,
                                                                  String remarks,
                                                                  String actor,
                                                                  boolean recordRegFormEvent) {
        String sn = clean(studentNumber);
        String rc = clean(reasonCode).toUpperCase();
        if (sn.isEmpty()) throw new IllegalArgumentException("Student number is required.");
        if (rc.isEmpty()) throw new IllegalArgumentException("Withdrawal reason is required.");
        ...
        double flatFee = resolveShiftCleanupFlatFee(sn);
        List<WithdrawalPolicySnapshot> flatPolicies = new ArrayList<>();
        for (int i = 0; i < policies.size(); i++) {
            WithdrawalPolicySnapshot base = policies.get(i);
            flatPolicies.add(flatPolicySnapshot(
                base,
                i == 0 ? flatFee : 0.0,
                i == 0 ? "Registrar flat shift-cleanup fee" : "Shift-cleanup archive line"));
        }
        ...
        if (flatFee > 0.01) {
            scholarEnrollmentService().postFlatDropCharge(
                sn,
                flatFee,
                "Registrar shift cleanup charge");
        }
        int residualPurged = purgeResidualCurrentTermEnlistments(sn, currentTermId, actedBy, "SHIFT_PROGRAM_CLEANUP");
        recordShiftCleanupEvents(sn, requestId, actedBy, currentLoad, flatFee, remarks, recordRegFormEvent);
        return new DirectDropResult(requestId, currentLoad.size() + residualPurged, flatFee, "SHIFT_PROGRAM_CLEANUP");
    }

    @Override
    @Transactional
    public void withdrawStudentForQueue(String studentNumber, String remarks, String actor) {
        try {
            dropStudentByRegistrar(studentNumber, "OTHER", remarks, actor);
        } catch (com.iuims.registrar.domain.forms.RegistrationFormSnapshotException e) {
            throw e;
        } catch (Exception ignored) {
        }
    }
    ...
    @Override
    @Transactional
    public void clearForShift(String studentNumber, String remarks, String actor) {
        try {
            clearCurrentTermLoadForProgramShift(studentNumber, remarks, actor);
        } catch (com.iuims.registrar.domain.forms.RegistrationFormSnapshotException e) {
            throw e;
        } catch (Exception ignored) {
        }
    ...
    private String validateSubjectDrop(String studentNumber) {
        ...
        if (!isOfficiallyEnrolledStudent(sn)) {
            throw new IllegalArgumentException("Current-term subject drop is available only for ENROLLED students.");
        }
        ScholarEnrollmentService scholarService = scholarEnrollmentService();
        if (scholarService != null && scholarService.hasAccountingBlock(sn)) {
            throw new IllegalArgumentException("Enrollment is blocked until the prior-term forwarded balance is settled at Cashier first.");
        }
        if (scholarService != null && scholarService.hasUnresolvedPendingCredit(sn)) {
            throw new IllegalArgumentException("Enrollment is blocked until the prior-term overpayment is resolved.");
        }
        String withdrawalBlock = withdrawalBlockMessage();
        if (withdrawalBlock != null) throw new IllegalArgumentException(withdrawalBlock);
        return sn;
    ...
    private List<Map<String, Object>> resolveSubjectDropBundle(String studentNumber,
                                                               Integer termId,
                                                               Map<String, Object> selectedEnlistment) {
        List<Map<String, Object>> currentLoad = findCurrentTermEnlistments(studentNumber, termId);
        if (!tableExists("course_corequisites")) {
            return List.of(selectedEnlistment);
        }
        LinkedHashSet<Integer> bundleCourseIds = resolveRequiredCorequisiteCourseIds(
            ((Number) selectedEnlistment.get("course_id")).intValue());
        List<Map<String, Object>> bundle = new ArrayList<>();
        for (Map<String, Object> enlistment : currentLoad) {
            if (!(enlistment.get("course_id") instanceof Number courseIdNumber)) {
                continue;
            }
            if (bundleCourseIds.contains(courseIdNumber.intValue())) {
                bundle.add(enlistment);
            }
        }
        return bundle.isEmpty() ? List.of(selectedEnlistment) : bundle;
    }

    private LinkedHashSet<Integer> resolveRequiredCorequisiteCourseIds(int courseId) {
    ...
    private int purgeResidualCurrentTermEnlistments(String studentNumber, Integer termId, String actor, String scope) {
        List<Map<String, Object>> residuals = findAllCurrentTermEnlistments(studentNumber, termId);
        if (residuals.isEmpty()) {
            return 0;
        }

        int purged = 0;
        for (Map<String, Object> row : residuals) {
            Object enlistmentId = row.get("enlistment_id");
            if (!(enlistmentId instanceof Number n)) {
                continue;
            }
            db.update("DELETE FROM student_enlistments WHERE enlistment_id = ?", n.longValue());
            purged++;
        }
        ...
        if (purged > 0) {
            ...
            documentTrailService.recordStudentEvent(
                ...
                "STUDENT",
                "WITHDRAWAL",
                "WITHDRAWAL_RESIDUAL_PURGED",
                "Residual current-term load cleaned up",
                String.format("%s cleanup removed %d lingering current-term enrolment row(s) after the main withdrawal pass.",
                    scope, purged),
                cleanActor(actor),
                null,
                "student_enlistments",
    ...
    private List<Map<String, Object>> findAllCurrentTermEnlistments(String studentNumber, Integer termId) {
        return db.queryForList("""
            SELECT se.enlistment_id, se.student_id, se.course_id, se.section_id, se.enlisted_date,
                   cs.term_id, c.course_code, c.course_title, c.credit_units, c.lec_units, c.lab_units,
                   COALESCE(c.component_type, 'SINGLE') AS component_type
            ...
            JOIN courses c ON c.course_id = se.course_id
            WHERE se.student_id = ? AND cs.term_id = ?
            ORDER BY c.course_code, se.enlistment_id
    ...
    private void recordShiftCleanupEvents(String studentNumber, long requestId, String actor,
                                          List<Map<String, Object>> enlistments, double totalCharge,
                                          String remarks, boolean recordRegFormEvent) {
        String courses = enlistments.stream()
            .map(row -> String.valueOf(row.get("course_code")))
            .reduce((left, right) -> left + ", " + right)
            .orElse("none");
    ...
    private boolean isOfficiallyEnrolledStudent(String studentNumber) {
        try {
            Map<String, Object> row = db.queryForMap(
                "SELECT admission_status FROM students WHERE student_number = ? LIMIT 1",
                studentNumber);
            String admissionStatus = row.get("admission_status") != null
                ? String.valueOf(row.get("admission_status"))
                : "";
            return "ENROLLED".equalsIgnoreCase(admissionStatus);
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private WithdrawalPolicySnapshot computePolicySnapshot(String studentNumber, Map<String, Object> enlistment) {
        ...
        DeadlineResolution deadline = resolveDropDeadline(termId);
        ...
        if (!deadline.configured()) {
            return new WithdrawalPolicySnapshot(
                requestedOn, enlistedAt, daysEnrolled, "DROP_DEADLINE_NOT_CONFIGURED",
                100.0, originalCost, true,
                deadline.message());
        }
        if (requestedOn.isAfter(deadline.deadlineDate())) {
            return new WithdrawalPolicySnapshot(
                requestedOn, enlistedAt, daysEnrolled, "AFTER_MIDTERM_BLOCKED",
                100.0, originalCost, true,
                String.format(
                    "Withdrawal deadline for %s is %s (%s). Requests are allowed only until the midterm end date.",
                    deadline.termLabel(), deadline.deadlineDate(), deadline.sourceLabel()));
        }
    ...
    private DeadlineResolution resolveDropDeadline(Integer termId) {
        String termLabel = resolveTermLabel(termId);
        if (termId == null || termId <= 0) {
            return new DeadlineResolution(termLabel, null, null,
                "Subject drop deadline is not configured for " + termLabel + ". Open Settings and set Midterm End.",
                false);
        }

        try {
            List<Map<String, Object>> rows = db.queryForList(
                "SELECT end_date FROM grading_term_windows WHERE term_id = ? AND grading_period = 'MIDTERM' LIMIT 1",
                termId);
            if (!rows.isEmpty()) {
                LocalDate deadline = toLocalDate(rows.get(0).get("end_date"));
                if (deadline != null) {
                    return new DeadlineResolution(termLabel, deadline, "grading_term_windows.MIDTERM_END", null, true);
                }
                return new DeadlineResolution(termLabel, null, "grading_term_windows.MIDTERM_END",
                    "Subject drop deadline for " + termLabel + " is not configured. Open Settings and set Midterm End.", false);
            }
        } catch (Exception ignored) {
        }
        ...
        try {
            String raw = db.queryForObject(
                "SELECT setting_value FROM system_settings WHERE setting_key = ? LIMIT 1",
                String.class, "MIDTERM_END");
            LocalDate deadline = raw != null && !raw.isBlank() ? LocalDate.parse(raw.trim()) : null;
            if (deadline != null) {
                return new DeadlineResolution(termLabel, deadline, "system_settings.MIDTERM_END", null, true);
            }
        } catch (Exception ignored) {
        }

        return new DeadlineResolution(termLabel, null, "system_settings.MIDTERM_END",
            "Subject drop deadline is not configured for " + termLabel + ". Open Settings and set Midterm End.", false);
    }

    private String resolveTermLabel(Integer termId) {
        if (termId == null || termId <= 0) {
            return "the current term";
        }
        try {
            Map<String, Object> row = db.queryForMap(
                "SELECT COALESCE(NULLIF(term_name, ''), term_code, CONCAT('term ', term_id)) AS term_label " +
                    "FROM academic_terms WHERE term_id = ? LIMIT 1",
                termId);
            String label = row.get("term_label") != null ? String.valueOf(row.get("term_label")) : "";
            return label.isBlank() ? ("term " + termId) : label;
        } catch (Exception e) {
            return "term " + termId;
        }
    }
    ...
    public DirectDropResult executeApprovedRequest(long requestId, String actor) {
        ...
        if ("FULL_CURRENT_TERM".equalsIgnoreCase(scope)) {
            subjectsDropped += purgeResidualCurrentTermEnlistments(
                studentNumber, requireCurrentTermId(), cleanActor(actor), "FULL_CURRENT_TERM");
    ...
    private WithdrawalPolicySnapshot flatPolicySnapshot(WithdrawalPolicySnapshot base, double flatFee, String note) {
        if (base == null) {
            return new WithdrawalPolicySnapshot(
                LocalDate.now(), null, 0, "FLAT", 100.0, Math.max(0.0, flatFee), false, note);
        }
        return new WithdrawalPolicySnapshot(
            base.requestedOn(),
            base.enlistedAt(),
            ...
            "FLAT",
            100.0,
            Math.max(0.0, flatFee),
            base.deadlineBlocked(),
            note != null && !note.isBlank() ? note : base.policyNote());
    }

    private double resolveFlatSubjectDropFee(String studentNumber) {
        return resolveConfiguredFlatFee("subject_drop_flat_fee", studentNumber);
    }

    private double resolveShiftCleanupFlatFee(String studentNumber) {
        double configured = resolveConfiguredFlatFee("shift_cleanup_flat_fee", studentNumber);
        if (configured > 0.01) {
            return configured;
        }
        return resolveConfiguredFlatFee("subject_drop_flat_fee", studentNumber);
    ...
    private double resolveConfiguredFlatFee(String settingKey, String studentNumber) {
        double configured = readEnrollmentSettingDouble(settingKey, Double.NaN);
        if (Double.isFinite(configured) && configured > 0.01) {
            return roundMoney(configured);
        }
        double fallback = resolveProgramAddDropFee(studentNumber);
        return roundMoney(fallback);
    }

    private double resolveProgramAddDropFee(String studentNumber) {
        if (studentNumber == null || studentNumber.isBlank()) {
            return 0.0;
        }
        try {
            Map<String, Object> student = db.queryForMap(
                "SELECT program_code, COALESCE(year_level, 1) AS year_level, COALESCE(semester, 1) AS semester " +
                    "FROM students WHERE student_number = ? LIMIT 1",
                studentNumber.trim());
            Integer programId = termFeeAdminService.resolveProgramId(
                student.get("program_code") != null ? String.valueOf(student.get("program_code")) : null);
            if (programId == null) {
                return 0.0;
            }
    ...
    private record DeadlineResolution(String termLabel,
