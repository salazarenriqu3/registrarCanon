...
public class CreditGradeService {
    ...
    private boolean isCoursePassed(String studentNumber, int courseId) {
        ...
        try {
            Integer count = db.queryForObject(
                "SELECT COUNT(*) FROM grades g WHERE " + equivalencyMatchClause(String.valueOf(courseId), "g.course_id")
                    + " AND " + in + " AND " + GradeOutcomeSql.passed("g"),
                Integer.class, args);
    ...
    private boolean courseInCurriculum(int curriculumId, int courseId) {
        try {
            Integer count = db.queryForObject(
                "SELECT COUNT(*) FROM curriculum_courses cc WHERE curriculum_id = ? AND "
                    + equivalencyMatchClause("cc.course_id", String.valueOf(courseId)),
    ...
    private boolean hasPendingRequest(String studentNumber, int courseId) {
        ...
        try {
            Integer count = db.queryForObject(
                "SELECT COUNT(*) FROM transfer_credit_requests WHERE student_number = ? AND status = 'PENDING' AND "
                    + equivalencyMatchClause(String.valueOf(courseId), "course_id"),
                Integer.class,
                studentNumber);
            return count != null && count > 0;
        } catch (Exception e) {
    ...
    private String equivalencyMatchClause(String canonicalCourseExpr, String candidateCourseExpr) {
        if (!tableExists("course_equivalencies")) {
            return canonicalCourseExpr + " = " + candidateCourseExpr;
        }
        return "(" + canonicalCourseExpr + " = " + candidateCourseExpr
            + " OR " + candidateCourseExpr + " IN (SELECT ce.equivalent_course_id FROM course_equivalencies ce "
            + "WHERE ce.course_id = " + canonicalCourseExpr + "))";
    }
        ...
        try {
            ...
            if ("applicant_credit_accreditation_lines".equalsIgnoreCase(sourceTable)) {
                ...
                db.update(
                    ...
                    postedGradeId,
                    Long.parseLong(sourceRowId));
                syncApplicantAccreditationHeaderApproval(Long.parseLong(sourceRowId), actor);
            }
        } catch (Exception ignored) {
        }
    }
    ...
    private void syncUpstreamRejection(Map<String, Object> row, String actor, String reason) {
        ...
        try {
            ...
            db.update(
                ...
                Long.parseLong(sourceRowId));
            syncApplicantAccreditationHeaderRejection(Long.parseLong(sourceRowId));
        } catch (Exception ignored) {
        }
    }
    ...
    private void syncApplicantAccreditationHeaderApproval(long lineId, String actor) {
        Long accreditationId = findApplicantAccreditationId(lineId);
        if (accreditationId == null) {
            return;
        }
        Integer nonApproved = db.queryForObject(
            "SELECT COUNT(*) FROM applicant_credit_accreditation_lines WHERE accreditation_id = ? AND COALESCE(registrar_decision_status, '') <> 'APPROVED'",
            Integer.class,
            accreditationId);
        if (nonApproved != null && nonApproved == 0) {
            db.update(
                "UPDATE applicant_credit_accreditations SET accreditation_status = 'APPROVED', approved_by = ?, approved_at = COALESCE(approved_at, NOW()), updated_at = NOW() WHERE accreditation_id = ?",
                cleanNullable(actor, 100),
                accreditationId);
        }
    }

    private void syncApplicantAccreditationHeaderRejection(long lineId) {
        Long accreditationId = findApplicantAccreditationId(lineId);
        if (accreditationId == null) {
            return;
        }
        db.update(
            "UPDATE applicant_credit_accreditations SET accreditation_status = 'ENDORSED', approved_by = NULL, approved_at = NULL, updated_at = NOW() WHERE accreditation_id = ?",
            accreditationId);
    }

    private Long findApplicantAccreditationId(long lineId) {
        try {
            return db.queryForObject(
                "SELECT accreditation_id FROM applicant_credit_accreditation_lines WHERE line_id = ? LIMIT 1",
                Long.class,
                lineId);
        } catch (Exception ignored) {
            return null;
    ...
    private void normalizeAccreditationCollations() {
        tryExecute("ALTER TABLE applicant_credit_accreditations CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        tryExecute("ALTER TABLE applicant_credit_accreditation_lines CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        tryExecute("ALTER TABLE transfer_credit_requests CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    }
