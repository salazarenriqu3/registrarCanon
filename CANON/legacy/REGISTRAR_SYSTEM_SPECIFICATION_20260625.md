...
For the ownership graph and term overlay rules, also read `2026-06-27_REGISTRAR_DEPENDENCY_MATRIX.md`.
...
- transfer crediting and program shifting as downstream official academic actions
...
- fee-readiness visibility for academic checks, not fee authoring
...
- official fee authoring and cashier/accounting policy ownership
- irregular/transferee or continuing-student pre-advising and draft generation
- production financial policy sign-off

In practice, Registrar owns academic structures, curriculum assignment, schedules, sections, and official academic records. Enrollment3/Cashier owns irregular and continuing-student draft generation, fee authoring, payment gates, and finalization; Registrar reads readiness and approves or posts academic outcomes where registrar authority is required.
...
| `program_fee_settings` exact scope | fee-readiness source by term + program + year + semester; official authoring belongs to Enrollment3 Accounting/Cashier |
...
- manual subject-add uses the student's explicit curriculum assignment and may reach across eligible curriculum year levels within the current semester; section eligibility is still guarded by prerequisites, conflicts, capacity, and max load
- a student may have no active current-term subjects and still use Add Subjects when an explicit current curriculum assignment exists
- program shift clears current-term load but must preserve or replace the explicit current curriculum assignment before the profile is considered ready
