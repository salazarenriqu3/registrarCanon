...
- The separate registrar feature demo overlay seeds concrete rooms for `BSIT-1-1-A`, `BSCPE-1-1-A`, and the block/special-section courses used in live demo flows.
...
1. Program Builder defines the core program record: code, name, department ownership, duration, and active/inactive status.
2. Course Builder defines reusable course catalog records, including lecture/laboratory unit splits and usage visibility before editing a shared course.
...
- Program Builder for core program master records, including department ownership, duration-years, status control, and usage inspection
- Course Catalog with shared course records, separate lecture/laboratory units, and usage drilldown before editing a reused course
...
- Student Manager subject add uses assigned curriculum as source of truth: same-semester courses across year levels stay eligible, while off-semester rows are blocked before enlistment
- class scheduling centers on block sections; the retired irregular-open-section language is replaced by summer/tutorial special sections
