...
| **Block offerings** + class scheduling | Done | Block Sections panel; special sections only for explicit exceptions |
...
| Audit logging | Medium | Started / key registrar actions wired | `audit_logs` now captures actor, module, action, target, summary, details, and source pointers for profile, documents, RegForm, curriculum assignment, scholarship workflow, scheduling, and slot monitoring. Dedicated audit browser UI is still pending. |
...
| Logins | `admin` / `1234`, `prof.cruz` / `1234`, `registrar.main` / `1234`, `registrar.records` / `1234`, `registrar.scholar` / `1234`, `registrar.schedule` / `1234` |
